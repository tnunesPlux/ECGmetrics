...
CS_table = zeros(length(limits), 2);
delete("CS.txt");
...
%     plot(timeAxis(Max), Ag_norm(Max), 'ko') ;
    ...
    for i = 1:length(CS)
        ...
        plot(Ag_Seg, 'r');
        CS(i) = (sum(Ag_Seg.*Dry_Seg)./(sqrt(sum(Ag_Seg.^2))*sqrt(sum(Dry_Seg.^2))));
        writelines(string(CS(i)), "CS.txt", LineEnding = "\n", WriteMode = "append");
        hold off;
...
%         hold on;
    ...
    CS_table(portion, :) = [sum(CS), length(CS)];
    fprintf('RMSE mean of portion %d is: %f\n', portion, mean(CS));

end

Final_RMSE = sum(CS_table(:, 1))/sum(CS_table(:, 2))

CS_values = importdata("CS.txt");
